...
const GradientGeneratorPage: React.FC = () => {
  ...
  const activeStop =
    colorStops.find((stop) => stop.id === activeStopId) || colorStops[0];
  ...
  const removeColorStop = (id: string) => {
    ...
    if (activeStopId === id) {
      setActiveStopId(
        colorStops[0].id === id ? colorStops[1].id : colorStops[0].id
      );
    }
  };
    ...
    <div className="p-4 space-y-8">
      <header>
        ...
        <p className="mb-6 text-gray-700 dark:text-gray-300">
          Create beautiful CSS gradients with a visual editor. Customize colors,
          positions, angles, and more.
        </p>
      </header>
      ...
      <div className="space-y-6">
                    ...
                    backgroundImage:
                      gradientType === 'linear'
                        ? `linear-gradient(${angle}deg, ${colorStops
                            ...
                            .map(
                              (stop) =>
                                `${stop.color.toHexString()} ${stop.position}%`
                            )
                            .join(', ')})`
        ...
        <section className="mt-8 p-4 border-2 border-border-color dark:border-dark-border-color shadow-solid dark:shadow-dark-solid">
          ...
          <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
            <p>
              <strong>CSS Gradients</strong> allow you to display smooth
            ...
            <ul className="list-none space-y-1 font-mono text-xs">
              <li>
                <span className="inline-block w-24">Linear:</span>{' '}
                <span className="text-gray-500">→</span> Colors flow in a single
                direction (angle)
              </li>
              <li>
                <span className="inline-block w-24">Radial:</span>{' '}
                <span className="text-gray-500">→</span> Colors radiate from a
                center point
              </li>
              <li>
                <span className="inline-block w-24">Conic:</span>{' '}
                <span className="text-gray-500">→</span> Colors rotate around a
                center point (not in this tool)
              </li>
            </ul>
            <p className="mt-2">
